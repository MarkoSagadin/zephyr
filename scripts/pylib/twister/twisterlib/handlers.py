...
class DeviceHandler(Handler):
    ...
    def _create_serial_connection(self, dut, serial_device, hardware_baud,
                                  flash_timeout, pty_name, ser_pty_process):
        try:
            ser = serial.Serial(
                serial_device,
            ...
            lprint("This is serial exception")
            self._handle_serial_exception(e, dut, pty_name, ser_pty_process)
            raise

        return ser


    def _handle_serial_exception(self, exception, dut, pty_name, ser_pty_process):
        self.instance.status = TwisterStatus.FAIL
        ...
        if pty_name and ser_pty_process:
            self._terminate_pty(pty_name, ser_pty_process)
    ...
    def get_hardware(self):
        ...
        try:
            ...
            in_waiting = 0
            while not hardware:
                lprint(f"Waiting for a DUT to run {self.instance.name}")
                time.sleep(1)
                in_waiting += 1
                if in_waiting%60 == 0:
        ...
        lprint(f"DUT ready to run: {hardware}")
        return hardware

    def _start_pty(self, command, pty_master):
        lprint("Starting serial pty with command: " + command)
    ...
    def handle(self, harness):
        ...
        lprint(f"Test name: {self.instance.name}")
        ...
        if hardware.flash_before is False:
            if serial_pty:
                ser_pty_process = self._start_pty(serial_pty, pty_master)
        ...
        try:
            stdout = stderr = None
            with subprocess.Popen(flash_command, stderr=subprocess.PIPE, stdout=subprocess.PIPE) as proc:
                try:
        ...
        if hardware.flash_before:
            try:
                if use_rtt:
                    lprint("Starting rtt after flashing")
                    rtt_command = self._create_rtt_command(hardware)
                    ser_pty_process = self._start_pty(rtt_command, pty_master)
                ...
                self._handle_serial_exception(e, hardware, pty_name, ser_pty_process)
                return
        ...
        lprint(f"RELEASING DUT: {hardware}")
