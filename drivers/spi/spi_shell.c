...
/* TODO: figure out if this number should be shared by both arrays */

#define NUM_EMPTY_SPI_THINGS 64
#define NUM_EMPTY_MAPS       64

/* Goals:
 * - user should be able to write any node name or node label and get access to an appropriate spi
 *   config.
 * - It shouldn't matter of that is a spi controller or a spi device.
 * - As long as they are in the device tree they should be accessible.
 * - Spi devices don't event have to be ok., as long the spi bus that they are on is ok then
 *   commands should work.
 * - Node names and node labels pointing to the same node should have the same spi_dt_spec struct.
 *
 *   It would be the easiest if all spi controllers and spi devices were in a single array, then
 *   you could just index into that array.
 *
 * node_label -> some_func -> name
 */
...
/* String representing either a spi bus or an spi device. */
struct spi_thing {
	/* Device name. Either a bus controller or spi device. */
	const char *name;
 ...
 * runtime. */
static struct spi_thing spi_things[] = {
	DT_FOREACH_STATUS_OKAY_NODE(INST_ALL_SPI_DEVICES_AS_SPI_THINGS)
	CREATE_NUM_EMPTY_STRUCTS(NUM_EMPTY_SPI_THINGS)
};
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(transceive, &dsub_spi_thing_name,
		      ...
		      "example: spi transceive spi@a000 0x00 0x01",
